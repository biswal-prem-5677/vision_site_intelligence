...
1. Detects and tracks personnel and vehicles in real time.
2. Evaluates multi-zone safety boundaries (`Crane Swing Area`, `Excavation Zone`, `Restricted Personnel Area`, `Equipment Operating Area`).
3. Provides an interactive 4-point visual polygon zone editor.
4. Classifies equipment motion into active vs. idle duration to calculate utilisation percentages.
5. Computes dynamic state-based site safety scores (0–100) and risk levels (`LOW`, `MODERATE`, `HIGH`).
6. Generates management-level executive reports via Gemini 2.0 Flash AI.
7. Persists structured telemetry and safety audit logs in a local SQLite database.
...
- **GitHub Repository Status**: Synchronized on branch `main` (`13b5867`)
